...
async function cancelOrders(marketPk: PublicKey, performAction: boolean) {
  ...
  if (performAction) {
    ...
    const filteredOrders = orders.data.orderAccounts.filter(
...
const processArgs = getProcessArgs(
  ["marketPk", "performAction"],
  "npm run cancelOrders"
);
cancelOrders(
  new PublicKey(processArgs.marketPk),
  boolFromString(processArgs.performAction)
);
