...
async function seedFromCsv(csvName: string, performAction: boolean) {
  ...
  outcomes.map((marketOutcome) => {
    ...
    const seedManager = SeedManager.initialize(
      parseFloat(marketOutcome.truePrice),
      parseFloat(marketOutcome.spread),
      parseFloat(marketOutcome.steps),
      parseFloat(marketOutcome.toReturn),
      parseFloat(marketOutcome.toLose),
      config.include_stake_in_returns,
      floatListFromString(marketOutcome.depthPercentages)
    );
    if (marketOutcome.seed === "TRUE") {
      log(
        `Market ${marketOutcome.marketPk} | Outcome: ${marketOutcome.outcome} | Title: ${marketOutcome.marketTitle}`,
        LOG_TYPE.INFO
      );
      log(`FOR`);
      ...
      if (performAction) {
        seedManager.forSeeds.map(async (seed) => {
          const seeding = await createOrderUiStake(
            program,
            marketPk,
            marketOutcome.outcomeIndex,
            true,
            seed.price,
            seed.stake
          );
