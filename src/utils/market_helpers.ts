...
import { getMarketOutcomeTitlesByMarket, getMarketAccountsByStatusAndMintAccount, MarketStatusFilter } from "@monaco-protocol/client";
...
export async function getOpenMarketsOutcomeMappingByToken(token: PublicKey){
    ...
    const marketsResponse = await getMarketAccountsByStatusAndMintAccount(program, MarketStatusFilter.Open, token)
    marketsResponse.data.markets.sort((a,b) => a.account.eventAccount.toString() > b.account.eventAccount.toString() ? 1: -1)
    const marketOutcomeMap = [] as MarketOutcomeMapping[]
    for (const market of marketsResponse.data.markets) {
        log(`Mapping outcomes for market: ${market.publicKey.toString()}`, LOG_TYPE.WAITING)
